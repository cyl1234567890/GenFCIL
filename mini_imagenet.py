...
class Mini_Imagenet:
    ...
    def get_data(self):
        ...
        class_path = [d for d in os.listdir(self.root) if os.path.isdir(os.path.join(self.root, d))]
        class_path.sort(key=lambda x: int(x))
        # print(class_path)
        ...
        train_list_img, test_list_img = np.array(train_list_img), np.array(test_list_img)

        train_list_label, test_list_label = np.array(train_list_label), np.array(test_list_label)
    ...
    def get_image_class_in_exemplar(self, label):
        class_samples = self.train_data[np.array(self.train_targets) == label]
        random_indices = np.random.choice(len(class_samples), size=10, replace=False)
        random_samples = class_samples[random_indices]
