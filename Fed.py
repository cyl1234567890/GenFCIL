...
def Feddel(net_glob, w_locals, gradient, idxs_users, max_now, dataset_test, test_sampler, args, test_count):
    ...
    for i in range(len(w_locals)):
        state.append(w_locals[i])


            
                    
#     print(loss_all, loss_pop, worker_ind)
    return state,w_locals, full_user, idxs_users, labeled, test_count
...
def Feddel_syn(net_glob, w_locals, gradient, idxs_users, max_now, dataset_test, args, test_count):#max_now:当前用户与所有用户的平均梯度进行内积运算
    ...
    while len(w_locals) > 8:
        ...
        if expect_list[key] <= expect_list["all"]:
            ...
            test_count[key][1] += 1
            expect_list.pop("all")
            loss_all, loss_pop, idxs_users= test_part(net_glob, w_locals, idxs_users, key, dataset_test, args)#loss_all：保留key的损失，loss_pop:去掉key的损失
            if loss_all < loss_pop:
                w_locals, idxs_users.append(key)
                break
            else:
